...
int main(int argc, char **argv) {
    ...
    XSetForeground(dpy, gc[1], BlackPixel(dpy, scr));
    XSetForeground(dpy, gc[2], BlackPixel(dpy, scr));
    XGCValues gval1;
    ...
    Window root = win; 
    win = XCreateWindow(dpy, root, 0, 0, WIN_WIDTH, WIN_HEIGHT, 1, CopyFromParent, InputOutput, CopyFromParent, amask, &attr);
    XSizeHints hint;
