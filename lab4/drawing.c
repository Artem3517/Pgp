...
void setCircleParam(Units *units, const XArc *tmp) {
    units->circles =
        (XArc *)((units->count == 0) ? calloc(1, sizeof(XArc))
                                     : realloc(units->circles, (units->count + 1) * sizeof(XArc)));
...
int checkBoarders(Units *units, XEvent *event) {
    ...
    while (i >= 0) {
        ...
        if (sqrt(pow(event->xbutton.x - x0, 2) + pow(event->xbutton.y - y0, 2)) <
            units->circles[i].width / 2) {
            del_circle = i;
            break;
        }
        i--;
...
void setFrame(XArc *tmp, int xm, int ym, const int *coords) {
    ...
    int rx = coords[0] - dr;
    int ry = coords[1] - dr;
    if (rx > 0 && ry > 0 && rx + 2 * dr < W && ry + 2 * dr < H) {
...
void drawFrame(Units *units, XArc *tmp) {
    ...
    XDrawArc(units->dpy, units->win, units->gc, tmp->x, tmp->y, tmp->width, tmp->height, tmp->angle1,
             tmp->angle2);
}
...
void redraw(Units *units) {
    ...
    for (int i = 0; i < units->count; i++) {
        ...
        XDrawArc(units->dpy, units->win, units->gc, units->circles[i].x, units->circles[i].y,
                 units->circles[i].width, units->circles[i].height, units->circles[i].angle1,
                 units->circles[i].angle2);
    }
}